...
#import "TSPToastView.h"
#import "TSPToastConfig.h"

@interface TSPToastView()
...
+ (instancetype _Nullable)toastWithMessage:(NSString * _Nullable)message
    ...
    TSPToastView *toastView = [[TSPToastView alloc] init];
...
- (void)setFrameWithMessage:(NSString * _Nullable)message
    ...
    CGFloat y = originY > 0 ? originY : ((KTSPScreenHeight - toastSize.height) / 2);
    CGFloat topSpace = kToastConfig.minTopMargin;
    y = y < topSpace ? topSpace : y;
    if (2 * topSpace + toastSize.height > KTSPScreenHeight) {
        toastSize.height = KTSPScreenHeight - (2 * topSpace);
    }
    CGFloat toastWidth = kToastConfig.minWidth > toastSize.width ? kToastConfig.minWidth : toastSize.width;
    self.frame = CGRectMake((kTSPScreenWidth - toastWidth) / 2, y, toastWidth, toastSize.height);
...
- (CGSize)getToastSizeWithMessage:(NSString * _Nullable)message
    ...
    CGFloat maxWidth = kTSPScreenWidth - (kToastConfig.minLeftMargin * 2);
    CGFloat maxHeight = KTSPScreenHeight - (kToastConfig.minTopMargin * 2);
